...
class WordConcatenation {
 public:
  static vector<int> findWordConcatenation(const string &str, const vector<string> &words) {
    ...
    string wordsInCurrentWindow;
    for(; windowEnd<str.length()  ; ++windowEnd)
    {
       wordsInCurrentWindow.push_back(str[windowEnd]);
       if(wordCountMap.find(wordsInCurrentWindow) != wordCountMap.end())
       {
           --wordCountMap[wordsInCurrentWindow];
       }
    }
  ...
  static vector<int> findWordConcatenationSolution(const string &str, const vector<string> &words) {
